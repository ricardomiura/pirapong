...
- **Placar:** um toque grava o vencedor (`partidas.vencedor`); em seguida aparece uma linha **opcional** de sets (3×0, 3×1, 3×2, 2×0, 2×1). O placar é gravado sempre na perspectiva dos lados `a`/`b` da partida, para o número não depender de quem venceu, e é exibido na perspectiva do vencedor. Trocar o vencedor **limpa** o placar, que passaria a contradizer o resultado.
- **Desempate, na ordem:** (1) vitórias; (2) mini-tabela — vitórias contando só os jogos entre os próprios empatados, o que com dois é o confronto direto e com três ou mais expõe ciclos (A ganha de B, B de C, C de A); (3) saldo de sets, também só entre os empatados. A coluna **S** na tabela mostra o saldo **geral** (é o que as pessoas esperam ver), mas o desempate usa o saldo **dentro do bloco empatado**, que é a regra correta — daí a legenda embaixo da tabela.
- **Quando nem isso fecha:** medindo por enumeração exaustiva, gravar só o vencedor deixa ~25% dos grupos travados; com placar em sets cai para ~6%. Nesse resto, `gerarChave()` **não classifica ninguém em silêncio**: pede confirmação explícita e só então `ordemFinal()` sorteia — e sorteia apenas entre os que estão exatamente empatados, cada um caindo numa vaga que o próprio empate já ocupava. Quem o critério separou nunca é mexido. O resultado do sorteio fica gravado nas partidas da chave.
